...
Resumen de los Cambios Realizados
    ...
    Corrección de Caracteres: Eliminé todos los caracteres con tilde (acentos) de los comentarios y de las cadenas de texto para asegurar la máxima compatibilidad (full ASCII).
    Flujo del Programa y Entradas: El problema de que el programa "se frena" se debe a cómo la función scanf deja caracteres residuales (como el 'Enter') en el búfer de entrada. He corregido esto agregando un limpiador de búfer while(getchar() != '\n'); después de cada scanf. Esto asegura que la siguiente lectura de datos no se salte pasos, haciendo el flujo del programa estable.
    Validación de Entradas Mejorada: Modifiqué la función registrarVenta para que, si el usuario ingresa un código o nombre de medicamento inválido:
    El sistema le informe del error.
    Muestre una lista de los códigos y nombres de medicamentos válidos para que pueda elegir correctamente.
    Le permita reintentar la búsqueda o ingresar 0 para cancelar y volver al menú principal.
*/
...
// --- Funciones de Utilidad y Busqueda ---
...
// NUEVO: Funcion para mostrar medicamentos disponibles y guiar al usuario
...
int main() {
    ...
    do {
        ...
        scanf("%d", &opcion);
        while(getchar() != '\n'); // CORREGIDO: Limpiar buffer de entrada SIEMPRE
...
void esperarEnter() {
    ...
    // CORREGIDO: Simplificado para evitar bloqueos. El buffer ya fue limpiado antes.
    getchar(); 
}
...
void ordenarMedicamentosPorCodigo(int codigos[], char nombres[][MAX_NOMBRE], double precios[], int stocks[], int esVentaLibre[], int stocksCriticos[], int n) {
    ...
    for (i = 1; i < n; i++) {
        ...
        char nombreActual[MAX_NOMBRE]; strcpy(nombreActual, nombres[i]);
...
int buscarMedicamentoPorNombre(const char* nombre, const char nombres[][MAX_NOMBRE], int n) {
    ...
    for(int i = 0; nombreBusqueda[i]; i++) nombreBusqueda[i] = tolower(nombreBusqueda[i]);
    for (int i = 0; i < n; i++) {
        strcpy(nombreActual, nombres[i]);
        for(int j = 0; nombreActual[j]; j++) nombreActual[j] = tolower(nombreActual[j]);
        if (strstr(nombreActual, nombreBusqueda) != NULL) return i;
    }
    return -1;
}

// NUEVO: Funcion para listar medicamentos y ayudar al usuario en la seleccion.
void listarMedicamentosDisponibles(const int codigos[], const char nombres[][MAX_NOMBRE], int n_medicamentos) {
...
// CORREGIDO: Funcion de venta completamente reestructurada para validar entradas.
void registrarVenta(int dia, int codigos[], const char nombres[][MAX_NOMBRE], double precios[], int stocks[], const int esVentaLibre[], double ventasDiarias[][2], char nombresReceta[][MAX_NOMBRE], int cantidadesReceta[], long long dnisReceta[], int* contadorVentasReceta, int n_medicamentos) {
    int tipoBusqueda;
    int idx = -1;
    ...
    // Bucle para reintentar la busqueda
    while (1) {
        if (tipoBusqueda == 1) {
            int codigo;
            ...
            scanf("%d", &codigo);
            while(getchar() != '\n'); // Limpiar buffer
            if (codigo == 0) return; // Salir al menu principal
            ...
            if (strcmp(nombre, "salir") == 0) return; // Salir al menu principal
        ...
        if (idx != -1) {
            break; // Medicamento encontrado, salir del bucle
        } else {
    ...
    // El resto de la logica de venta
...
void emitirInformeDiario(int dia, const double ventasDiarias[][2]) {
    limpiarPantalla(); // CORREGIDO: Limpiar pantalla para mejor visualizacion
...
void generarInformeMensual(const double ventasDiarias[][2], char* buffer) {
    ...
    // CORREGIDO: Limpiar pantalla para mejor visualizacion
    limpiarPantalla();
    strcpy(buffer, "========= INFORME MENSUAL DE VENTAS =========\n");
    strcat(buffer, "Dia       |  Ventas ($)  | Cant. Transacciones\n");
    strcat(buffer, "---------------------------------------------\n");
